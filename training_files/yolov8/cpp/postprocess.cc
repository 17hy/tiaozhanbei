...
static int infer_branch_count(int output_count, int* output_per_branch)
{
    if (output_count % 3 == 0)
    {
        int branch_count = output_count / 3;
        if (branch_count >= 3 && branch_count <= 5)
        {
            *output_per_branch = 3;
            return branch_count;
        }
    ...
    if (output_count % 2 == 0)
    {
        int branch_count = output_count / 2;
        if (branch_count >= 3 && branch_count <= 5)
        {
            *output_per_branch = 2;
            return branch_count;
...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    // Support both the default P3/P4/P5 heads and the P2/P3/P4/P5 small-object layout.
#ifdef RKNPU1
    ...
    int branch_count = infer_branch_count(app_ctx->io_num.n_output, &output_per_branch);
    if (branch_count < 0)
    {
        printf("Unsupported output layout: %d tensors\n", app_ctx->io_num.n_output);
        return -1;
    }
