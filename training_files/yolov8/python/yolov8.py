...
def infer_head_layout(output_count):
    """Infer detection-head count from RKNN outputs."""
    for output_per_branch in (3, 2):
        if output_count % output_per_branch != 0:
            continue
        branch_count = output_count // output_per_branch
        if 3 <= branch_count <= 5:
            return branch_count, output_per_branch
    raise ValueError(f"Unsupported YOLO output layout: {output_count} tensors")

def post_process(input_data):
    boxes, scores, classes_conf = [], [], []
    branch_count, output_per_branch = infer_head_layout(len(input_data))
    # Python 忽略 score_sum 输出
    for i in range(branch_count):
        boxes.append(box_process(input_data[output_per_branch * i]))
        classes_conf.append(input_data[output_per_branch * i + 1])
        scores.append(np.ones_like(input_data[output_per_branch * i + 1][:, :1, :, :], dtype=np.float32))
